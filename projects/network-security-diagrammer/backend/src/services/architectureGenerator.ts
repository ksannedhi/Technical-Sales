...
const GENERATION_SYSTEM_PROMPT_LINES = [
  ...
  "- Monitoring zones must always appear after enforcement zones of the same zone type. Set sortPriority: 10 on any zone whose label or purpose is monitoring/observability. This guarantees the monitoring zone renders below enforcement zones even when both have the same zone type.",
  ...
  "- NETWORK SEGMENTATION topology (prompts about isolated segments, VLANs, micro-segmentation, air-gapped zones, trust tiers): segments are ISOLATED from each other — each segment connects independently to a central enforcement/firewall component; segments must NEVER be chained to one another. Correct topology: Users/Internet (top) → Enforcement Zone (firewall/switch) → [Segment A | Segment B | Segment C] (bottom, side-by-side). Wrong topology: Segment A → Firewall → Segment B → Firewall → Segment C (linear chain implies segments communicate with each other, which contradicts the segmentation purpose). Use a single enforcement zone with multiple parallel connections, one to each segment.",
  "",
  "NAMING RULES:",
  ...
  "- Include a securityRationale array of exactly 3 concise bullet strings explaining the key security design decisions. Each bullet must name a specific component or zone from your output and explain WHY it is positioned or connected the way it is — not generic principles. Bad: 'Firewalls provide perimeter security.' Good: 'WAF in the DMZ inspects inbound HTTPS before it reaches the app tier, limiting blast radius to the edge if exploited.'",
  ...
  "10. Segmentation check: if the architecture contains multiple segment zones (VLANs, trust tiers, air-gapped zones, micro-segments), verify there are NO direct connections between any two segment components. Every segment must connect only to the enforcement/firewall component, not to other segments. If segment-to-segment connections exist, remove them — they defeat the segmentation model.",
  "",
